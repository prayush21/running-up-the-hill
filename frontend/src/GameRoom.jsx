    ...
    useEffect(() => {
            ...
            setGuesses(prev => {
                ...
                if (existingIndex !== -1) {
                    newGuesses[existingIndex] = {
                        ...
                        duplicateTrigger: guess.player_name === playerName ? Date.now() : newGuesses[existingIndex].duplicateTrigger
                    };
        ...
        socket.on('player_joined', (data) => {
            ...
            if (data.players) {
                ...
                setPlayers(prev => prev.includes(data.player_name) ? prev : [...prev, data.player_name]);
            }
        });
            ...
            {loadingMsg && (
                <div className="fixed inset-0 z-[200] bg-blueprint-dark/90 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="text-center">
                        ...
                        <h2 className="text-2xl font-game font-bold text-cream tracking-widest uppercase mb-2">Setting up Game</h2>
                        <p className="text-emerald-400 font-mono text-sm animate-pulse">{loadingMsg}</p>
                    </div>
                </div>
            )}
            <header className="fixed top-0 inset-x-0 z-50 glass-panel border-x-0 border-t-0 border-b border-blueprint-light p-4 flex items-center justify-between shadow-xl">
                <div className="flex items-center space-x-4">
                    ...
                    <div className="relative">
                        ...
                        >
                            <span className="text-xs text-cream-dark opacity-60 mr-2 uppercase tracking-wide hidden sm:inline">Room</span>
                            <span className="font-mono text-accent font-bold tracking-widest">{roomId}</span>
                            <svg className={`w-4 h-4 ml-2 text-cream-dark transition-transform ${showPlayers ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                        </button>
                        ...
                        {showPlayers && (
                            <div className="absolute top-full mt-2 left-0 min-w-[200px] glass-panel border border-blueprint-light rounded-lg shadow-[0_10px_30px_-10px_rgba(26,58,109,0.8)] overflow-hidden z-50 animate-slide-up">
                                <div className="px-4 py-2 bg-blueprint-dark/80 border-b border-blueprint-light flex justify-between items-center">
                                    <span className="text-xs font-game uppercase text-cream-dark opacity-80">Team Players</span>
                                    <span className="text-xs font-mono bg-accent/20 text-accent px-2 py-0.5 rounded-full">{players.length}</span>
                                </div>
                                <ul className="max-h-48 overflow-y-auto bg-blueprint-dark/40 backdrop-blur-md">
                                    {players.map((p, i) => (
                                        <li key={i} className="px-4 py-2.5 border-b border-blueprint-light/30 last:border-0 flex items-center space-x-3 hover:bg-blueprint-light/10 transition-colors">
                                            <div className="relative">
                                            ...
                                            {p === playerName && <span className="text-[10px] uppercase text-emerald-400/80 ml-auto border border-emerald-400/30 px-1.5 py-0.5 rounded font-bold">You</span>}
                                        </li>
                                    ))}
                                </ul>
                            </div>
            ...
            <div className="sticky top-20 z-40 w-full mb-8 mt-24">
                <div className="max-w-xl mx-auto text-center mb-4">
                    <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-blueprint-dark/80 border border-blueprint-light backdrop-blur-sm shadow-[0_0_15px_rgba(26,58,109,0.5)]">
                        <span className="text-xs uppercase tracking-widest text-cream opacity-70 font-mono">Team Guesses</span>
                        <span className="text-sm font-game font-bold text-accent">{guesses.length}</span>
                    </div>
                </div>
            ...
            {winningWord && (
                <div className="fixed inset-0 z-[100] bg-blueprint-dark/80 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="glass-panel p-8 sm:p-12 rounded-2xl max-w-lg w-full text-center border-accent border-2 animate-pulse-border shadow-[0_0_50px_rgba(255,140,0,0.3)] max-h-[90vh] flex flex-col">
                        <h2 className="text-4xl font-game font-bold text-accent mb-2 shrink-0">Victory!</h2>
                        <p className="text-xl text-cream mb-4 shrink-0">
                            The daily word was <span className="font-bold underline italic ml-1 block text-3xl mt-4 text-emerald-400 uppercase tracking-widest">{winningWord}</span>
                        </p>
                        ...
                        {top10Nearest && (
                            <div className="mb-6 text-left overflow-y-auto pr-2 custom-scrollbar flex-1 min-h-0">
                                <h3 className="text-sm tracking-widest uppercase font-game text-accent/80 mb-3 text-center border-b border-blueprint-light pb-2">Top 10 Nearest Words</h3>
                                <ul className="space-y-2">
                                    {top10Nearest.map((w, i) => (
                                        <li key={i} className="flex justify-between items-center text-cream font-mono text-sm bg-blueprint-dark/50 px-4 py-2 rounded border border-blueprint-light/30">
                                            <span><span className="text-emerald-400/70 mr-2">{w.rank}.</span> {w.word}</span>
                                            <span className="text-emerald-400">{(w.similarity * 100).toFixed(2)}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
